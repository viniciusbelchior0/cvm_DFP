#ATENÇÃO: SCRIPT RECOMENDADO PARA VERSÕES DO BANCO DE DADOS SEM A INCLUSÃO DA DEMONSTRAÇÃO DO FLUXO DE CAIXA (DFC)
...
setwd("C:/Users/user/Desktop/cvm_DFPs")
...
obter_dfp <- function(ano){
  ...
  planilhas <- unzip(paste0("C:/Users/user/Desktop/cvm_DFPs/dfp_cia_aberta_",ano,".zip"))
...
inserir_dados_dfps <- function(df){
  con <- dbConnect(RPostgres::Postgres(), dbname = "teste_dados_r",
                   host = "localhost", port = "5432",
                   user = "postgres", password = "admin") # Estabelecer conexão com o banco de dados PostgreSQL
  ...
  #Obtendo lista de todas as companhias já existentes no BD e filtrando apenas novas
  todas_dfps <- dbGetQuery(con,"SELECT * FROM demonstracoes")
  df <- df %>% anti_join(todas_dfps, by = "cia_ano")
  
  if (nrow(df) > 0) {
    dbWriteTable(conn = con, name = "demonstracoes", value = df,
                 row.names = FALSE, overwrite = FALSE, append = TRUE)
    cat("Dados inseridos com sucesso.")} else {cat("Sem novas demonstracoes financeiras. Nenhum dado inserido")}
  
  dbDisconnect(con) # Fechar a conexão com o banco de dados
}

inserir_dados_cias(cias)
inserir_dados_dfps(demonstracoes)
